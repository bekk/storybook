...
const Container = styled.div`
  ...
  &:hover {
    path {
      ...
      animation-name: ${fade};
      animation-timing-function: cubic-bezier(0.77, 0.3, 0.39, 0.9);
...
// JSX.IntrinsicElements sine keys er alle gyldige html-elementer (som a, article etc)
type HtmlElements = keyof JSX.IntrinsicElements;

// Om man sender inn en string i `as`-propen, så skal den godta de propsa de ellers ville tatt i mot
// Dette fungerer ikke av en eller annen grunn (alle props er lov på vanlige HTML-elementer)
type BuiltInHtmlElements<P extends HtmlElements> = JSX.IntrinsicElements[P] & {
  as: P;
};

// Om man sender inn en React-komponent, så skal den godta de propsa som den komponenten ellers ville tatt i mot
type CustomReactComponents<P> = P & {
  as: React.ComponentType<P>;
};

// Dette er mye typer for lite - men man kan sende inn en optional `as` prop, som enten kan være en string eller en React-komponent.
type Props<P, Q extends HtmlElements> =
  | BuiltInHtmlElements<Q>
  | CustomReactComponents<P>
  | { as?: undefined };

export function BekkLogo<T, Q extends HtmlElements>(props: Props<T, Q>) {
  return (
